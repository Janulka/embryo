...
/* --- Changes : ---
  ...
  Wish List
    ...
    o writing data depending on timing in a more smart way, e.g. using 10% 
...
char *
cmaes::cmaes_SayHello(cmaes_t *t) {
...
double *
cmaes::cmaes_init(cmaes_t *t, /* "this" */
                  int dimension,
                  double *inxstart,
  ...
  if (t->sp.typicalXcase)
    for (i = 0; i < N; ++i)
      t->rgxmean[i] += t->sigma * t->rgD[i] * t->rand.gauss();
...
void
cmaes::cmaes_resume_distribution(cmaes_t *t, char *filename) {
  int i, j, res, n;
  double d;
...
/* --------------------------------------------------------- */
double const *
cmaes::cmaes_SetMean(cmaes_t *t, const double *xmean)
/*
...
/* --------------------------------------------------------- */
double * const *
cmaes::cmaes_SamplePopulation(cmaes_t *t) {
  ...
  for (iNk = 0; iNk < t->sp.lambda; ++iNk) { /* generate scaled random vector (D * z)    */
    for (i = 0; i < N; ++i)
      t->rgdTmp[i] = t->rgD[i] * t->rand.gauss();
...
/* --------------------------------------------------------- */
double const *
cmaes::cmaes_ReSampleSingle_old( cmaes_t *t, double *rgx) {
  int i, j, N=t->sp.N;
  double sum;
...
/* --------------------------------------------------------- */
double * const *
cmaes::cmaes_ReSampleSingle( cmaes_t *t, int index) {
  int i, j, N=t->sp.N;
  double *rgx;
  double sum;
  static char s[99];
...
/* --------------------------------------------------------- */
double *
cmaes::cmaes_SampleSingleInto( cmaes_t *t, double *rgx) {
  int i, j, N=t->sp.N;
  double sum;
...
/* --------------------------------------------------------- */
double *
cmaes::cmaes_UpdateDistribution( cmaes_t *t, const double *rgFunVal) {
...
//  for (int ii = 0; ii < t->sp.lambda; ii++)
//      printf("%e | ", rgFunVal[t->index[ii]]);
//  printf("\n");
  ...
  for(i = *(t->arFuncValueHist-1)-1; i > 0; --i) /* for(i = t->arFuncValueHist[-1]-1; i > 0; --i) */
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
void cmaes::cmaes_WriteToFile(cmaes_t *t, const char *key, const char *name) {
  cmaes_WriteToFileAW(t, key, name, "a"); /* default is append */
}

/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
void cmaes::cmaes_WriteToFileAW(cmaes_t *t, const char *key, const char *name,
                                char *appendwrite) {
  char *s = "tmpcmaes.dat";
  FILE *fp;
...
/* --------------------------------------------------------- */
void 
cmaes::cmaes_WriteToFilePtr(cmaes_t *t, const char *key, FILE *fp)
...
/* --------------------------------------------------------- */
double
cmaes::cmaes_Get( cmaes_t *t, char const *s) {
  int N=t->sp.N;
...
/* --------------------------------------------------------- */
double *
cmaes::cmaes_GetInto( cmaes_t *t, char const *s, double *res) {
  int i, N = t->sp.N;
...
/* --------------------------------------------------------- */
double *
cmaes::cmaes_GetNew( cmaes_t *t, char const *s) {
  return (cmaes_GetInto(t, s, NULL));
}

/* --------------------------------------------------------- */
const double *
cmaes::cmaes_GetPtr( cmaes_t *t, char const *s) {
  int i, N=t->sp.N;
...
*/
const char *
cmaes::cmaes_TestForTermination( cmaes_t *t) {
  double range, fac;
  int iAchse, iKoo;
...
/* --------------------------------------------------------- */
void
cmaes::cmaes_ReadSignals(cmaes_t *t, char const *filename) {
...
/* --------------------------------------------------------- */
void
cmaes::cmaes_ReadFromFilePtr( cmaes_t *t, FILE *fp)
...
/* --------------------------------------------------------- */
void
cmaes::cmaes_UpdateEigensystem(cmaes_t *t, int flgforce) {
  int i, N = t->sp.N;
...
/* ========================================================= */
void
cmaes::cmaes_FATAL(char const *s1, char const *s2, char const *s3,
                   char const *s4) {
